...
struct Encoder{
	...
	dynet::Expression build_graph(dynet::ComputationGraph &cg, const WordIdSentences& ssents/*batch of sentences*/, ModelStats* pstats=nullptr){
		// compute source (+ position) embeddings
...
struct DecoderLayer{
	...
	std::pair<dynet::Expression, dynet::Expression> build_graph_getpair(dynet::ComputationGraph &cg
			, const dynet::Expression& i_enc_inp
			, const dynet::Expression& i_dec_inp
			, const MaskBase& self_mask
			, const MaskBase& src_mask)
	{
		...
		dynet::Expression i_ctx_decl = i_decl;
		...
		return make_pair(i_ctx_decl, i_decl);
	}
            ...
            , vector<dynet::real>& vec_s)
    {
        ...
        /*
        std::vector<dynet::real> i_ctx_td = as_vector(cg.incremental_forward(i_decl));//(num_units x Ly) x 1 i.e. flattens the tensor

        auto& d = i_decl.dim();
        unsigned b = d[0] * d[1];
        unsigned steps = d[0];//num_units

        unsigned t;
        for (unsigned i = 0; i < b; i+=steps){//to recreate the matrix containing representations
            t = i / steps;
            std::vector<dynet::real> ctx_rep(i_ctx_td.begin() + i, i_ctx_td.begin() + i + steps);
        }
        vec_c = ctx_rep;//contains the representation for the last column only
        */
        // position-wise feed-forward sub-layer
        dynet::Expression i_ff = _feed_forward_sublayer.build_graph(cg, i_decl);// ((num_units, Ly), batch_size)
        ...
        //std::vector<dynet::real> i_tgt_td = as_vector(cg.incremental_forward(i_decl));//(num_units x Ly) x 1 i.e. flattens the tensor

        //for (unsigned i = 0; i < b; i+=steps)//to recreate the matrix containing representations
        //    std::vector<dynet::real> word_rep(i_tgt_td.begin() + i, i_tgt_td.begin() + i + steps);
        //vec_s = word_rep;//contains the representation for the last column only
...
struct Decoder{
			...
			, const dynet::Expression& i_src_rep)
	{
		...
		// compute the decoder representation
		dynet::Expression i_dec_l_out = i_tgt_rep;
		dynet::Expression i_ctx_l_out;
		for (auto& dec : _v_dec_layers){
			// stacking approach
			tie(i_ctx_l_out, i_dec_l_out) = dec.build_graph_getpair(cg, i_src_rep, i_dec_l_out, _self_mask, _src_mask);// each position in the decoder can attend to all positions (up to and including the current position) in the previous layer of the decoder.
		}

		return make_pair(i_ctx_l_out, i_dec_l_out);// ((num_units, Ly), batch_size)
	}
    ...
    dynet::Expression build_graph(dynet::ComputationGraph &cg
            ...
            , vector<dynet::real>& vec_s)
    {
        // compute target (+ position) embeddings
        dynet::Expression i_tgt_rep = compute_embeddings_and_masks(cg, tsents);// ((num_units, Ly), batch_size)
...
public:
    ...
    dynet::Expression build_graph(dynet::ComputationGraph &cg
		...
		, ModelStats* pstats=nullptr
		, bool is_eval_on_dev=false);
    //for getting representations (added by Sameen Maruf)
    ...
    WordIdSentence greedy_decode(dynet::ComputationGraph& cg, const WordIdSentence &source, unsigned length_ratio=2);// greedy decoding at test time (revised by Sameen Maruf)
...
//for bilingual representations from the decoder side (added by Sameen Maruf)
std::vector<std::pair<std::vector<dynet::real>, std::vector<dynet::real>>> TransformerModel::compute_bilingual_rep(dynet::ComputationGraph& cg, const WordIdSentence &source, unsigned length_ratio)
{
